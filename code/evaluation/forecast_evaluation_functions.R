  ...
  if (!is.null(rac_map) && unit_level_name == "county") {
    ...
    geo_wide_mapping <- geo_wide_mapping %>%
      left_join(
        rac_level %>%
          mutate(rac = location) %>%
          dplyr::select(unit_id, rac),
        by = "unit_id"
      ) %>%
      left_join(
        dshs_level %>%
          mutate(dshs_region = location) %>%
          dplyr::select(unit_id, dshs_region),
        by = "unit_id"
      ) %>%
      left_join(
        hsa_level_from_county %>%
          mutate(hsa = location) %>%
          dplyr::select(unit_id, hsa),
        by = "unit_id"
      )
  }
...
run_cluster_eval <- function(date_list,
  ...
  obs <- read.csv(
    file.path("data/cluster_data_season", paste0("df_", method_name, "_exclude_", season, "_", n_cluster, ".csv"))
  )
  
  obs_all <- read.csv(
    file.path("data/cluster_data_season", paste0("df_", method_name, "_exclude_", season, "_", n_cluster, "_all.csv"))
  )
  
  output_folder <- paste0("TX_NSSP_", method_name, "_", season, "_", n_cluster, "_pct")
